<?php
//Theme options
include (TEMPLATEPATH . '/inc/includes/hirad-widget.php');
if (!function_exists( 'optionsframework_init')) {
	require_once(get_template_directory() . '/inc/options-framework.php');
}
if ( function_exists('register_sidebar') ) {
   register_sidebar(array(
      'name' => 'Home Sidbar',    // نامی سایدبار
      'id' => 'home-sidbar',         // آیدی سایدبار رو در اینجا تعیین کنید
...
if ( function_exists('register_sidebar') ) {
   register_sidebar(array(
      'name' => 'setting sidbar',    // نامی سایدبار
      'id' => 'setting-sidbar',         // آیدی سایدبار رو در اینجا تعیین کنید
      'description' => 'my new sidebar', // توضیحی در مورد این سایدبار
      'before_widget' => '<div class="widget">',   // کد قبل از هر منو
      'after_widget' => '</div>',                         // کد بعد از هر منو
      'before_title' => '<h2 class="widget-title">', // قبل از عنوان منو
 ...
 function add_theme_scripts() {
  wp_enqueue_style( 'semantic', 'https://cdn.jsdelivr.net/semantic-ui/2.2.9/semantic.min.css', array(), '', 'all' );
   wp_enqueue_style( 'bootstrap', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css', array(), '', 'all' );
   wp_enqueue_style( 'font-awesome', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css', array(), '', 'all' );
  wp_enqueue_style( 'dropdown', get_template_directory_uri() . '/css/dropdown.css', array(), '1.1', 'all');

  wp_enqueue_style( 'mosaic.css', get_template_directory_uri() . '/css/mosaic.css', array(), '1.1', 'all');
  
  
   wp_enqueue_style( 'animate', 'https://cdnjs.cloudflare.com/ajax/libs/animate.css/3.5.2/animate.min.css', array(), '', 'all' );
    wp_enqueue_script( 'modernizr', get_template_directory_uri() . '/js/modernizr.custom.js', array ( 'jquery' ), 1.1, true);

 wp_enqueue_script( 'hirad-js', get_template_directory_uri() . '/js/hirad-js.js', array ( 'jquery' ), 1.1, true);

  wp_enqueue_script( 'semantic.min', get_template_directory_uri() . '/js/semantic.min.js', array ( 'jquery' ), 1.1, true);
  ...
  wp_enqueue_script( 'mosaic', get_template_directory_uri() . '/js/mosaic.js', array ( 'jquery' ), 1.1, true);
	 wp_enqueue_script( 'cbpshop', get_template_directory_uri() . '/js/cbpShop.js', array ( 'jquery' ), 1.1, true);
...
function ipin_after_setup_theme() {
	load_theme_textdomain('ipin', get_template_directory() . '/prslangs');
...
function ipin_widgets_init() {
	register_sidebar(array('id' => 'sidebar-r-t', 'name' => 'ستون کناری صفحه وصله ها (بالای تخته ها)', 'before_widget' => '<div class="sidebar-wrapper"><div class="sidebar-inner">', 'after_widget' => '</div></div>', 'before_title' => '<h4>', 'after_title' => '</h4>'));
	register_sidebar(array('id' => 'sidebar-r', 'name' => 'ستون کناری صفحه وصله ها (زیر تخته ها)', 'before_widget' => '<div class="sidebar-wrapper"><div class="sidebar-inner">', 'after_widget' => '</div></div>', 'before_title' => '<h4>', 'after_title' => '</h4>'));
	register_sidebar(array('id' => 'sidebar-others', 'name' => 'ستون کناری صفحات دیگر', 'before_widget' => '<div class="sidebar-wrapper"><div class="sidebar-inner">', 'after_widget' => '</div></div>', 'before_title' => '<h4>', 'after_title' => '</h4>'));
}
...
function ipin_setup_guide() {
	if (function_exists('add_options_page'))
		add_theme_page('آماده سازی پوسته', 'آماده سازی پوسته', 'edit_theme_options', 'theme_installation', 'ipin_setup_guide_page');
}
...
<div class="wrap">
	<?php screen_icon(); ?>
    <h2>آماده سازی پوسته</h2>
	<hr />
    <table class="form-table"><tr><th>
		<div style="background: #fcfcfc; border: 1px solid #eee; padding: 15px; max-width: 550px;">
			<strong>نیازمندی های سرور</strong>
			<ul>
				<li>PHP Extension: Curl 
				<?php if (extension_loaded('curl')) { ?>
				<span style="color: green; font-weight: bold; font-style:italic;">فعال</span>
				<?php } else { $error_extension = true; ?>
				<span style="color: red; font-weight: bold; font-style:italic;">غیر فعال!</span>
				<?php } ?>
				</li>
				...
				<span style="color: green; font-weight: bold; font-style:italic;">قابل نوشتن</span>
				<?php } else { ?>
				<span style="color: red; font-weight: bold; font-style:italic;">غیر قابل نوشتن!</span>
				<?php } ?>
				</li>
				
				<?php if ($error_extension ) { ?>
				<p><span style="color: red; font-weight: bold; font-style:italic;">اخطار:</span> افزونه های ضروری PHP فعال نمی باشند. لطفاً با مدیریت سرور خود در تماس باشید.</p>
				<?php } ?>
				
				<?php if (!is_writable(WP_CONTENT_DIR)) { ?>
				<p><span style="color: red; font-weight: bold; font-style:italic;">اخطار:</span> پوشه WP-Content (<?php echo WP_CONTENT_DIR; ?>) قابل نوشتن نمی باشد. لطفاً دسترسی پوشه را برروی 755 یا 777 قرار دهید. اگر دسترسی لازم را برای اینکار ندارید، لطفاً با مدیریت سرور خود در تماس باشید.</p>
				<?php } ?>
				
				<?php if (!$error_extension && is_writable(WP_CONTENT_DIR)) { ?>
				<p>پشتیبانی نیازمندی های سرور تائید شد. لطفاً فرآیندهای زیر را نیز انجام دهید.</p>
				<?php } ?>
			</ul>
		</div>
		...
		<ol>
			<li>
				از آدرس <strong><a href="<?php echo admin_url('options-general.php'); ?>" target="_blank">تنظیمات > همگانی</a></strong> تنظیمات زیر را انجام دهید:
				<ul>
					<li>عضویت = هر کسی می‌تواند نام‌نویسی کند (تیک بزنید)</li>
					<li>نقش پیش‌فرض کاربر تازه = نویسنده</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('options-reading.php'); ?>" target="_blank">تنظیمات > خواندن</a></strong> تنظیمات زیر را انجام دهید:
				<ul>
					<li>بیشترین تعداد نوشته‌ها در هر برگه‌ی وبلاگ = 20 (یا هر تعداد که دوست دارید. 20 برای یک شروع خوب)</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('options-media.php'); ?>" target="_blank">تنظیمات > رسانه</a></strong> تنظیمات زیر را انجام دهید:
				<ul>
					<li>اندازه‌ی میانه: بیشترین پهنا = 235، بیشترین ارتفاع = 4096</li>
					<li>اندازه‌ی بزرگ: بیشترین پهنا = 700، بیشترین ارتفاع = 4096</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('options-permalink.php'); ?>" target="_blank">تنظیمات > پیوندهای یکتا</a></strong> تنظیمات زیر را انجام دهید:
				<ul>
					<li>ساختار دل‌خواه = /pin/%post_id%/</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('edit-tags.php?taxonomy=category'); ?>" target="_blank">نوشته‌ها > دسته‌ها</a></strong> انجام دهید:
				<ul>
					<li>دسته بندی های خود را بیافزائید برای مثال: مشاهیر، تغذیه، تکنولوژی</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('nav-menus.php'); ?>" target="_blank">نمایش > فهرست‌ها</a></strong> انجام دهید:
				<ul>
					<li>از جعبه "دسته‌ها"، فهرست هایی که ساخته اید را با کلیک برروی "افزودن به فهرست" درون فهرست قرار دهید. موارد اضافه شده جدید را با کشیدن، همتراز گزینه "همه چیز" کنید.</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('themes.php?page=options-framework'); ?>" target="_blank">نمایش > گزینه های پوسته</a></strong> انجام دهید:
				<ul>
					<li>تنظیمات دلخواه خود را اعمال کنید</li>
					<li>بیاد داشته باشید که نکات مهم هر گزینه را مطالعه فرمائید</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php global $current_user, $wp_rewrite; echo home_url('/' . $wp_rewrite->author_base . '/' . $current_user->data->user_nicename . '/'); ?>" target="_blank"><?php echo home_url('/' . $wp_rewrite->author_base . '/' . $current_user->data->user_nicename . '/'); ?></a></strong> انجام دهید:
				<ul>
					<li>اگر صفحه 404 را مشاهده کردید، به آدرس <a href="<?php echo admin_url('options-permalink.php'); ?>" target="_blank">تنظیمات > پیوندهای یکتا</a> رفته و به آسانی برروی دکمه "ذخیره تغییرات" کلیک نمائید.</li>
				</ul>
			</li>
			
			<li>
				فایل <strong>wp-config.php</strong> را ویرایش کنید:
				<ul>
					<li>فایل wp-config.php که برروی سرور وردپرس شما قرار دارد را ویرایش کنید، عبارت <em>define('WP_DEBUG', false);</em> را جستجو کرده و در زیر آن عبارت <em>define('EMPTY_TRASH_DAYS', 0);</em> را اضافه کنید.</li>
				</ul>
			</li>
			
			<li>
				افزونه <strong>WP Social Login</strong> را نصب کنید (اختیاری - برای ورود از طریق شبکه های فیسبوک و توئیتر)
				<ul>
					<li>آدرس <a href="<?php echo admin_url('themes.php?page=options-framework'); ?>" target="_blank">نمایش > گزینه های پوسته</a> > را وارد کرده و از برگه یادداشت ها > افزونه های توصیه شده، افزونه ها را نصب فرمائید.</li>
				</ul>
			</li>
			
			<li>
				<strong>از پوسته خود لذت ببرید! یا اگر می خواهید وبلاگ هم داشته باشید ادامه آموزش را نیز مطالعه کنید (اختیاری).</strong>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('edit-tags.php?taxonomy=category'); ?>" target="_blank">نوشته‌ها > دسته‌ها</a></strong> انجام دهید:
				<ul>
					<li>دسته ای جدید را اضافه کنید. برای مثال: وبلاگ</li>
				</ul>
			</li>
			...
			<li>
				از آدرس: <strong><a href="<?php echo admin_url('themes.php?page=options-framework'); ?>" target="_blank">نمایش > گزینه های پوسته</a></strong> انجام دهید:
				<ul>
					<li>گزینه "دسته بندی برای وبلاگ"، را برروی دسته ای که به همین منظور ساخته اید قرار دهید.</li>
				</ul>
			</li>
			...
			<li>
				از آدرس <strong><a href="<?php echo admin_url('nav-menus.php'); ?>" target="_blank">نمایش > فهرست‌ها</a></strong> انجام دهید:
				<ul>
					<li>در جعبه "دسته‌ها"، دسته موردنظر برای وبلاگ را با کلیک برروی "افزودن به فهرست" به فهرست بیافزائید.</li>
				</ul>
			</li>
			
			<li>
				از آدرس <strong><a href="<?php echo admin_url('post-new.php'); ?>" target="_blank">نوشته‌ها > افزودن نوشته</a></strong>
انجام دهید:				<ul>
					<li>نوشته ای ایجاد کرده و مطمئن گردید که در دسته ای که برای وبلاگ ساخته اید، منشر شود.</li>
				</ul>
			</li>
		</ol>
    </th></tr></table>
